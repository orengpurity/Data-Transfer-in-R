...
# ------------------------------------------------------------------------------
## Introduction 
# R script is a plain text file that contains R code and comments
# They end with the file extension .R
# Other ways to store Codes and comments uses R Markdown and Quarto files
...
# Load data
# Data analysis 
# R comments and codes 

# ------------------------------------------------------------------------------
## Types of scripts
# R scripts (extension file is .R)
# Markdown files (extension file is .Rmd)
# Quarto files (extension file is .qmd)
...
# #YAML headers: always end and begin with three dashes (---) typed on its own line
# Has the title, date, type of document (HTML, Word, PDF) information after rendering

# #R code chunks: r codes are saved as chunk codes that starts with three back ticks (```), 
...
# #Markdown: all of the other text is called Markdown used to write comments and
...
# organizing our files into R projects allows us to use relative file paths,
# instead of absolute file paths, 
# which will make it much easier for to collaborate with others
...
# Passes the results of one function to the next after another (nesting) and 
# gives the final last output from the last command
# should always have a space in front of it and after

# Example
lpi <- seq(from = 2, to = 100, by = 2) |> log() |> mean()
lpi
...
height_and_weight
filter(height_and_weight, sex == "Female") # fishes out the two rows
...
# Types of file formats include;
# database files, spreadsheets, text files, SAS data sets, and Stata data sets

# R can store data in two formats;
# a) Manually create dataframes using data.frames(), tibbles() or
# tribble() functions
# b) Import data file in R (raw data)

# Note; manual ways are not always very practical 
#– particularly when using have larger data sets
...
# 2nd; On a Mac, right-click on the file we want the path for and a drop-down
# menu will appear. Then, click the Get Info menu option, copy the file path in 
# the Where section of the get info window and paste it into our R code.
# On windows, hold down the shift key and right click on the file we want 
# the path for. Then, click Copy as path in the drop-down menu that appears 
# and paste the file path into our R code.

# 2. Relative paths
# Importing excel spreadsheets into R uses read_excel() function
# E.g., 
# data <- read_excel (copy-paste path to PC- either relative or absolute)

# Note; for absolute paths, work on the R project of interest and write the path
# E.g., data <- read_excel (mydata/purelabs.csv)
...
# simple files that you (a human) can directly read using only your operating system’s 
# plain text editor (i.e., Notepad on Windows or TextEdit on Mac) with .txt file extension
# They also include commonly shared as CSV (comma separated values) files (.csv file extension)
# or as Microsoft Excel files (.xls or .xlsx file extensions)
...
# more complex file types that can’t generally be read by humans without 
# the use of special software
# Some examples include Microsoft Excel spreadsheets, SAS data sets, and Stata data sets

# # Note; 
# R can only manipulate and analyze data that has been imported into 
...
# Replacing . with NA for missing values
# By passing that information to the na argument of the read_delim() function
single_space <- read_delim(
  file = "data/single_delimited.txt",
  delim = " ",
  na = "."
...
tab <- read_delim(
  file = "data/tab.txt",
  delim = "\t" # means “tab” to R
)
tab

# OR; can also use tab specific function of read_tsv()
# does exactly the same thing as the read_delim() function with the value of the
# delim argument set to "\t"
tab <- read_tsv("data/tab.txt")
tab

# iii).  Importing fixed width format files (fwf)
# use readr’s read_table() function to import data
...
# col_positions =; tells R the width (i.e., number of columns) that belong
# to each variable in the raw data set from the fwf_widths() function
# skip =; argument tells R how many rows to ignore before looking for
# data values in the raw data

# b. Paired vector of start and end positions

# Tell R how which columns each variable starts and stops at in the raw data.
# uses read_fwf() function
...
# b). haven; used to import SAS and Stata data sets
...
# c. col_names =; telling R to use the words in the character vector as column names,
# instead of the values in the first row of the raw data (the default)
?readr::cols
# d. col_types =; tells R explicitly what type of values you want each column to contain
# by nesting a col_ function for each column type inside the col() function, 
# which is passed directly to the col-types argument
...
#Some of the national, population-based public health surveys,
#(e.g., BRFSS and NHANES) make their data publicly available in this format
...
# 4. find and select the csv file, 

# After selecting you file, there will be some changes in the data import window.
...
# At this point;
# you can change the variable type by:
# clicking in the column headers in the Data Preview field,
# a dropdown menu will display that allows you to change variable types
# Note; This is equivalent to adjusting the default values passed to the,
# col_types argument of the read_csv() function.
...
## run the code as though you wrote it, by;
# # click the clipboard icon to copy the code displayed in the Code Preview window,
# and then click the Cancel button
# # return to your R script or R Markdown file and paste the code

#======================================================================================

# ## Exporting Data ##

# Normally, data frames are stored in environment temporarily, from one 
# programming session to the next

# Aim; learn to export data from R’s memory to a file on your hard drive so that,
# you may efficiently store it or share it with others
...
# 2.R binary files
# uses readr package and exports in both .Rdata and the .Rds formats
